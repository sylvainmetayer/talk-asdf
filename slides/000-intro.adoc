include::001-presentation.adoc[]

== Problématique

[.column]
--
.There are so many of them...
video::so_many_of_them_lotr.mp4[width=1200,opts="autoplay,loop,muted"]

// TODO https://github.com/asciidoctor/asciidoctor-reveal.js/issues/523
--

[.notes]
****
Côté devops, et programmation de manières générale, pleins d'outils qui évolue parfois vite, avec autant d'outils que de distribution linux.

Et on se retrouve avec des projets qui suivent niveau versions et d'autres qui restent sur des vieilles versions.

Et il faut pourtant utiliser les 2
****

[.columns.transparency]
== Contexte

// image::devoxx/DevoxxFR2024_0053.jpg[background, size=fill]

[.column.context_success]
--
include::../code/0-go-new.txt[]
--

[.column.context_error.fragment]
--
include::../code/0-go-old.txt[]
--

[.transparency]
== Solution

// image::devoxx/DevoxxFR2024_0011.jpg[background, size=fill]

image::asdf.png[alt='']

[.notes]
****
1. Installation
2. Concepts
3. Usage
4. Création d'un plugin
****